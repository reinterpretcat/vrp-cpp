...
SCENARIO("Can create joint convolution pair from two convolutions", "[convolution][join_pairs]") {
  ...
  Pool pool;
  std::vector<Convolution> left = { {0,  1, 1, {1, 4}, {}, {1, 4} }, {0,  2, 2, {7, 13}, {}, {7, 13} },
                                    {0, 3, 3, {15, 19}, {}, {15, 19} } };
  std::vector<Convolution> right  { {21, 4, 4, {1, 6}, {}, {1, 6} }, {21, 5, 5, {6, 11}, {}, {6, 11} } };


  auto result = create_joint_convolutions {} (problem, tasks, {0.2, 0.2, 1, pool}, map(left, pool), map(right, pool));


  compare(result->operator[](0), { 4, static_cast<float>(6)  / customers, { left.at(0), right.at(0) } });
  compare(result->operator[](1), { 0, static_cast<float>(10) / customers, { left.at(0), right.at(1) } });

  compare(result->operator[](2), { 0, static_cast<float>(13) / customers, { left.at(1), right.at(0) } });
  compare(result->operator[](3), { 5, static_cast<float>(8)  / customers, { left.at(1), right.at(1) } });

  compare(result->operator[](4), { 0, static_cast<float>(11)  / customers, { left.at(2), right.at(0) } });
  compare(result->operator[](5), { 0, static_cast<float>(11)  / customers, { left.at(2), right.at(1) } });
}
