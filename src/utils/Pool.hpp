...
public:
  ...
  public:
    ...
    ptr_type acquire() {
      ...
      ptr_type tmp(obj,
                   [this](void* ptr) { this->release(std::unique_ptr<T>(static_cast<T*>(ptr))); });

      if (!pool_.empty()) pool_.pop();

      return std::move(tmp);
    }

    void release(std::unique_ptr<T> t) { pool_.push(std::move(t)); }
...
private:
  ...
  std::shared_ptr<TypedPool<T>> pool() {
    auto it = poolMap_.insert(
      std::make_pair(std::type_index(typeid(T)), std::shared_ptr<void>(new TypedPool<T>())));
