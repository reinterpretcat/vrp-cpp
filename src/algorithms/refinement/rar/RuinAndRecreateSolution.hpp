...
struct ruin_and_recreate_solution final {
  models::EstimatedSolution operator()(const RefinementContext& ctx, const models::Solution& sln) const {
    // TODO how to pass settings?
    auto newSln = std::make_shared<models::Solution>(Recreate{}(ctx, Ruin{}(ctx, sln)));
